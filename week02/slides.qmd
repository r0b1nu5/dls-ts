...
Consider a company with 10 employees and 1 boss. 
All employees have the same monthly salary of CHF 3'000.- and the boss has a CHF 30'000.- monthly salary. 

Let us compute the average and variance of salaries within this company.

```{pyodide}
salary = np.concatenate((np.full(10,3000),[30000]))

salary.mean()
```

## Weakness of the mean
And how about a similar company where employees are paid CHF 5'000.- monthly and the boss is paid CHF 10'000.-?

```{pyodide}
salary = np.concatenate((np.full(10,5000),[10000]))
...
for i in range(len(rax.columns)):
 for j in range(len(rax.columns)):
  ax.text(j, i, f'{rax.iloc[i, j]:.2f}', ha='center', va='center', color='black')
